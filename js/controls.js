...
function PlayerController(player, playPauseBtn, stopBtn, rwdBtn, fwdBtn, timeLabel) {
    ...
    this.playPauseBtn.onclick = function() {
        if (player.paused) {
            player.play();
            playPauseBtn.setAttribute('class', 'fa-duotone fa-pause');
            /*playPauseBtn.textContent = 'Pause';*/

        } else {
            player.pause();
            playPauseBtn.setAttribute('class', 'fa-duotone fa-play');
...
for (let i = 0; i < players.length; i++) {
    ...
    /* const iplay = document.createElement('i');
     playpause.appendChild(iplay);*/

    playpause.setAttribute('class', 'fa-duotone fa-play');
